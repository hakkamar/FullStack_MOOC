...
function voittaja(taulukko) {
    let suurin = 0    
    for (let index = 0; index < 10; index++) {
        if (taulukko[index] > taulukko[suurin]) {
            suurin = index
        }
    }
    return suurin
  ...
  render() {
    return (
      <div>
        ...
        <div> {this.props.anecdotes[voittaja(this.state.pisteet)]} </div>
        <div> has {this.state.pisteet[voittaja(this.state.pisteet)]} votes </div>
      </div>
    )
  }
}
